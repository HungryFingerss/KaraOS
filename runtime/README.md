...
The middle layer of the P1.A1 decomposition of the former 9k-line `pipeline.py` monolith. **Layering direction (AST-enforced): `flows/` to `runtime/` to `core/`, nothing here imports `pipeline`.** `pipeline.py` re-exports these symbols so the historical import surface and the test suite stay byte-compatible.
...
| `vision_loop.py` | the background vision loop (detect to track to recognize to stores) |

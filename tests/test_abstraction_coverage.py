...
def test_load_spacy_handles_load_error_and_sets_flag(monkeypatch, capsys):
    # lines 54-58: spacy imports fine but .load raises OSError (model not
    # downloaded) -> logged, flag set, None returned. A fake `spacy` module is
    # injected so this runs headless (real spacy is an OPTIONAL dep, absent in
    # the fast-CI env); the same lines catch ImportError there naturally.
    import sys
    import types
...
def test_real_spacy_end_to_end_abstracts_person():
    ...
    # proving the fake-nlp assumptions about doc.ents match reality. spacy +
    # the model are an OPTIONAL dep; the NER branch is already covered headless
    # by the fake-nlp tests above, so skip cleanly when they're not installed.
    if _load_spacy() is None:
        pytest.skip("spacy + en_core_web_sm not installed (optional dep)")

...
def _reset_session_state_between_tests():
    """Reset SessionStore between every test — prevents session state leakage.

    autouse=True ensures this runs for every test in the suite.
    setup_pipeline_stubs() is idempotent; calling it here means P0.7 tests
    never need to import pipeline.py directly.
    """
    setup_pipeline_stubs()
    from core.session_state import SessionStore  # noqa: PLC0415
    # Reset the module-level _session_store if pipeline has one (P0.7.2+).
    # In P0.7.1, pipeline doesn't have _session_store yet — the try/except

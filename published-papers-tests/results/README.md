...
| [`multilayer_classifier_ablation/`](multilayer_classifier_ablation/) | 16-run ablation of three retrieval improvements layered on top of the production graph classifier. Phase A (6 stages establishing fair baseline + isolating each sub-improvement), Phase B (5-run adaptive K threshold sweep), Phase C (4-run hierarchical fallback sweep), Phase D (combined winners). All 16 runs published, including the regressions and the parameter-sweep flatlines that revealed design issues. | **+0.56pp** from distance-weighted voting + Layer 1 quality gating (production_equivalent 0.6464 → 0.6520). Adaptive K (-1pp) and hierarchical retrieval (-7.8pp) did not pay off as designed; both sweeps documented that the parameters tested were past their useful operating range. Honest negative findings published with diagnosis. |

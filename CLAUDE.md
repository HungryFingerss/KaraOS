...
3. **Pre-P1 Bundle 2 Plan v2 auditor-catches-architect (VENDORED-MIT-IN-D6-SCOPE sub-variant; PI #3)** — architect's Plan v2 §2 D6 mechanical-script applied uniform `SPDX-License-Identifier: LicenseRef-KaraOS-Proprietary` to all 204 in-scope files including 2 vendored MIT-licensed files (`core/_minifasnet/__init__.py` + `model.py`). Auditor's Plan v2 review caught the licensing-precision violation per REUSE Software spec semantics. Plan v3 absorbed via `EXCLUDED_PATHS = ("core/_minifasnet/",)` constant + Bundle 2.X concurrent MIT LICENSE file.

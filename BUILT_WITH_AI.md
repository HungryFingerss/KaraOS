...
- Plan v1 to v2 documents incorporating auditor feedback
...
- **Tool-timeout structural invariants**, induced a synchronous `.execute()` loop without `await asyncio.sleep(0)` checkpoint (would break transaction rollback on `wait_for` cancellation) and flipped `include_tools=False` to `True` on the retry path (would re-enable tools recursively). Both AST-based invariants fired correctly.
...
- **Event log + replay harness**, full event-sourcing foundation across 9 staged steps. Phase 0 boundary audit to Plan v1 with locked D1-D8 decisions to Plan v2 with R1-R5 mid-spec refinements to code phase. 12 typed payload dataclasses, exactly-one-producer-per-event-type AST invariant, 11 producer hooks at 12 sites, read-only replay CLI, reusable scenario fixtures consumed by the next sub-PR's regression tests, health-log integration via observability counters.

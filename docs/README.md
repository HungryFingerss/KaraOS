...
| Item | What it is |
|---|---|
| `architecture/` | **The full architecture reference**, 19 chapters (section 1-section 340) split from the original monolithic doc, with a CI-enforced section-number stability invariant (every sectionNN lives in exactly one chapter; duplicates fail the build). Start at `architecture/README.md` for the chapter index. The root `everything_about_system.md` is a thin redirect into this tree. |

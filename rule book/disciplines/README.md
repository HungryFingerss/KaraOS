...
**Phase 0 audit (read-only, grep-verified) to Plan v1 to auditor gate to Plan v2 (absorb PIs) to developer build (step-by-step, architect-checkpointed) to auditor closure-audit to architect Layer-3 to commit to tracker bank to closure report.**
...
Every structural invariant ships with its non-vacuity proof: **break the thing it protects to the test must fire FOR THE PRODUCTION REASON to restore net-zero** (exact-reverse edit, NEVER `git checkout` on an uncommitted tree). Mid-flight detector strengthening from a RED that didn't fire is the protocol *working*, not scope creep.

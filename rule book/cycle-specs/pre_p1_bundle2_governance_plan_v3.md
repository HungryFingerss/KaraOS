...
Plan v2 §2 D6 mechanical-script applied uniform `SPDX-License-Identifier: LicenseRef-KaraOS-Proprietary` to all 204 files. Auditor PI #3 identified this as a **licensing precision violation**: `core/_minifasnet/__init__.py` + `core/_minifasnet/model.py` are MIT-licensed (Copyright 2020 Minivision) per upstream `minivision-ai/Silent-Face-Anti-Spoofing`. Inline copyright notices preserve the MIT attribution; uniform Apache-2.0 SPDX header would either misrepresent the license, assert unauthorized sublicensing, or create ambiguous declaration (inline says MIT; header says Apache-2.0 → REUSE-tooling conflict).
